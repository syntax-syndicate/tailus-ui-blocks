...
const sizes:ToggleRootProps["size"][] = ["xs", "sm", "md", "lg", "xl"]

type Variant = {
    variant: ToggleRootProps["variant"],
    sizes : ToggleRootProps["size"][]
}
        ...
        <div className="py-4 space-y-4">
            {
                variants.map((variant, index) => (
                    <div key={index} className="h-12 flex items-center">
                        <Caption className="capitalize">{variant.variant}</Caption>
                    </div>
                ))
            }
        </div>
                ...
                <div className="px-4 grid gap-4 grid-cols-5">
                    {
                        sizes.map((size, index) => (
                            <Caption key={index} className="capitalize">{size}</Caption>
                        ))
                    }
                </div>
                <div className="border p-4 grid gap-4 rounded-[--card-radius]">
                        ...
                        <div key={index} className="flex items-end gap-4">
                            {
                                variant.sizes.map((size, index) => (
                                    <Toggle.Root key={index} data-shade="900" variant={variant.variant} size={size} aria-label="Toggle bold">
                                        <Toggle.Icon>
                                            <Bold />
                                        </Toggle.Icon>
                                    </Toggle.Root>
