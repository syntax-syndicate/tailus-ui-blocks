...
export const Calendar = ({ className, classNames, intent = 'primary', fancy = false, handDrawn = false, showOutsideDays = true, ...props }: CalendarProps) => {
    const range = props.mode ? props.mode === 'range' : false;

    const { months, month, caption, caption_label, nav, nav_button, nav_button_previous, nav_button_next, table, head_row, head_cell, row, cell, day, day_range_start, day_range_end, day_selected, day_today, day_outside, day_disabled, day_range_middle, day_hidden } = calendar({ range });
