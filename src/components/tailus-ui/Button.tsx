...
export const Root = React.forwardRef<HTMLButtonElement & HTMLAnchorElement, ButtonProps>(({ className, intent = 'primary', variant = 'solid', size = 'md', disabled, href, children, ...props }, forwardedRef) => {
    const Component = href ? 'a' : 'button';
    const iconOnly = React.Children.toArray(children).some((child) => React.isValidElement(child) && child.type === Icon && child.props.type === 'only');
