...
export const Link: React.FC<LinkProps> = ({
    ...
    return(
        <a href={to} target={target && '_blank'} className={cn(linkVariants({ size, className }), target &&  "group flex items-center gap-0.5")} {...props}>
            {label}
            {children}
            {
                target ? 
                    <GoTopRightIcon className='size-3.5 text-primary-600 dark:text-primary-400 group-hover:text-primary-600 dark:group-hover:text-primary-400 group-hover:translate-x-px group-hover:-translate-y-px' /> : 
                    ""
            }
        </a>
    )
