...
export const Link = ({ isActive = false, label, link, children }: { isActive?: boolean; children?: ReactNode; label?: string; link: string }) => {
    return (
        <Button.Root href={link} variant={isActive ? 'outlined' : 'ghost'} intent="gray" className={twMerge('justify-start gap-3.5 px-4', isActive && 'bg-white dark:bg-gray-500/10 dark:!shadow-none dark:[--btn-border-color:theme(colors.transparent)]')} aria-label={label}>
