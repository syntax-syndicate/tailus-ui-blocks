---
title: Switch
description: A control that allows the user to toggle between checked and not checked.
priorityOrder : 53
frameworkDocs : 
    name : Radix UI
    link : https://www.radix-ui.com/primitives/docs/components/swicth#api-reference
---

import _components from "@content/_components"
import DocsStackedSnippets from "@components/DocsStackedSnippets"
import ComponentPreview from "@components/ComponentPreview"
import PreviewBlock from "@components/PreviewBlock.astro"
import CodeSnippet from "@components/utilities/CodeSnippet"
import Table from "@components/Table.astro"

import { Overview, overviewCode } from "@react-ui/switch/Overview"
import { checkboxProps, alignerProps } from "@react-ui/checkbox/reference"
import { installation } from "@react-ui/switch/codeSnippets"
import { aligner } from "@react-ui/checkbox/codeSnippets"

export const components = {
    ..._components
}

export const usage = [
    {
        code: `import Switch from "@tailus-ui/Switch";
import Label from "@tailus-ui/Label";
import Aligner from "@tailus-ui/Aligner";
import { Caption } from "@tailus-ui/typography";`,
        lang: "tsx",
    },
    {
        code: `const MyComponent = () => (
    <Aligner fromRight className="max-w-md">
        <Label htmlFor="airplane-mode">
            Airplane mode
        </Label>
        <Switch.Root className="mt-1" id="airplane-mode">
            <Switch.Thumb />
        </Switch.Root>
        <Caption as="p" size="base">Switch components are used to allow users to toggle between two states, such as on and off.</Caption>
    </Aligner>
)`,
        lang: "tsx",
    }
]

<div className="space-y-20">

    <PreviewBlock codeBlocks={overviewCode}>
        <Overview client:visible/>
    </PreviewBlock>

    <div>
        ## Installation

        Install the primitive and Copy-Paste the component code in a `.tsx` file.

        <DocsStackedSnippets snippets={installation} client:visible/>

        <div className="mt-12">

            ### Add Aligner component

            Useful for aligning `Checkbox`, `RadioGroup`, and `Switch` components with their associated `Label` and `Caption`

            <CodeSnippet {...aligner} client:visible/>
        </div>
    </div>

    <div>
        ## Usage

        Import all the parts and build your Switch.
        
        <DocsStackedSnippets snippets={usage} client:visible/>
    </div>

    <div>
        ## Reference

        <div className="mt-6 mb-12 space-y-12"> 
            
            <div>
                ### Root
            
                The Radio Group `<Root />` has the following additional props.

                <Table data={checkboxProps}/>
            </div>
            
            <div>
                ### Aligner
                
                The Aligner component has the following props.

                <Table data={alignerProps}/>
            </div>
        </div>
    </div>

    <div>
        ## Examples

        <div className="space-y-12">
            <div>
                
            </div>
        </div>
    </div>
</div>